struct Uniforms {
    view: mat4x4f,
    projection: mat4x4f,
    time_size_width: vec4f,
    viewport_height: f32,
};
@group(0) @binding(0) var<uniform> uniforms: Uniforms;

const light_point = vec3f(150.0, 70.0, 0.0);
...
fn snoise(v: vec3f) -> f32 {
    ...
    let vCy = dot(v, C.yyy);
    var i: vec3f = floor(v + vec3f(vCy, vCy, vCy));
    let iCx = dot(i, C.xxx);
    let x0 = v - i + vec3f(iCx, iCx, iCx);

    // Other corners
    let g = step(x0.yzx, x0.xyz);
    let l = (vec3f(1.0, 1.0, 1.0) - g).zxy;
    let i1 = min(g, l);
    let i2 = max(g, l);
    ...
    // Permutations
    var temp: vec3f = 289.0 * one.xyz;
    i = modf(i, &temp);
    let p = permute(
        permute(
            permute(i.zzzz + vec4f(0.0, i1.z, i2.z, 1.0))
    ...
    var m: vec4f = max(0.6 * one - vec4f(dot(x0, x0), dot(x1, x1), dot(x2, x2), dot(x3, x3)), 0.0 * one);
    m *= m;
    return 9.0 * dot(m*m, vec4f(dot(p0, x0), dot(p1, x1), dot(p2, x2), dot(p3, x3)));
}

// End of 3D simplex code.

fn apply_distortion(pos: vec3f) -> vec3f {
    var perlin_pos: vec3f = pos;
    ...
    perlin_pos = vec3f(perlin_pos.y - perlin_pos.x - size, perlin_pos.x, perlin_pos.z);
    ...
    // Rotate Time Y
    let perlin_pos_y = vec3f(xcos + zsin, perlin_pos.y, -xsin + xcos);

    // Rotate Time Z
    let perlin_pos_z = vec3f(xcos - ysin, xsin + ycos, perlin_pos.x);

    // Rotate 90 Y
    perlin_pos = vec3f(perlin_pos.z - perlin_pos.x, perlin_pos.y, perlin_pos.x);

    // Rotate Time X
    let perlin_pos_x = vec3f(perlin_pos.x, ycos - zsin, ysin + zcos);
...
fn make_position(original: vec2f) -> vec4f {
    let interpreted = vec3f(original.x * 0.5, 0.0, original.y * Y_SCL);
    return vec4f(apply_distortion(interpreted), 1.0);
}

// Create the normal, and apply the curve. Change the Curve Bias above.
fn make_normal(a: vec3f, b: vec3f, c: vec3f) -> vec3f {
...
fn calc_fresnel(view: vec3f, normal: vec3f) -> f32 {
...
fn calc_specular(eye: vec3f, normal: vec3f, light: vec3f) -> f32 {
...
struct VertexOutput {
    ...
    @location(0) f_WaterScreenPos: vec2f,
    @location(1) f_Fresnel: f32,
    @location(2) f_Light: vec3f,
};
...
fn vs_main(
    ...
    let p_pos = vec2f(position);
    let b_pos = make_position(p_pos + vec2f(offsets.xy));
    let c_pos = make_position(p_pos + vec2f(offsets.zw));
    let a_pos = make_position(p_pos);
    let original_pos = vec4f(p_pos.x * 0.5, 0.0, p_pos.y * Y_SCL, 1.0);
    ...
    let transformed_light = vm * vec4f(light_point, 1.0);
    ...
    result.f_WaterScreenPos = (0.5 * gridpos.xy * (1.0 / gridpos.w)) + vec2f(0.5, 0.5);
...
const water_colour = vec3f(0.0, 0.46, 0.95);
const zNear = 10.0;
const zFar = 400.0;
...
fn fs_main(vertex: VertexOutput) -> @location(0) vec4f {
    ...
    let normalized_coords = vertex.position.xy / vec2f(uniforms.time_size_width.w, uniforms.viewport_height);
    ...
    let depth_colour = mix(final_colour, water_colour, vec3f(t, t, t));

    return vec4f(depth_colour, clamped * (1.0 - vertex.f_Fresnel));
}
